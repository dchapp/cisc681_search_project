...
path to all goals is built by finding the subpath between a start point and the
nearest goal.  This is accomlished through iterative calls to the A* algorithm
while changing the start position.  Psuedocode for this multi-objective
heuristic can be found in algorithm~\ref{nna}.

\begin{algorithm}
    \caption{Na\"{\i}ve multi-objective A*}
    \label{nna}
    \begin{algorithmic}[1]
...
\begin{algorithm}
    \caption{Combinatorial multi-objective A*}
    \label{ca}
    \begin{algorithmic}[1]
