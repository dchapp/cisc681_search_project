...
consistently underestimate the actual cost from a position to a goal node.  The
first heuristic calculates the manhattan distance between a position and a goal
node.  The second heuristic calculates the euclidean distance between the two
points.  If there is more than one goal, the heuristics will calculate the
distance to each goal and return the distance to the nearest goal.  Pseudocode
for each heurstic can be found below.
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State $(x1, y1)\gets start$
        \For{each point $goal$ in $goals$}
        \State $(x2, y2)\gets goal$
        \State $dist\gets \lvert x2-x1\rvert + \lvert y2-y1\rvert$
